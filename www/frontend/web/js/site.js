...
function openStartPage() {
	var pageType = jQuery("#asModal").attr('data-page');
	if (pageType=='modal' && !jQuery("#asModal").hasClass("show")) {
		closeAllModal();
		getActiveButton(2);
...
jQuery(document).ready(function($) {
	...
	$(".app-back").on("click", function() {
		...
		var backPage = $(this).parents(".modal").attr("data-parent");
		if (typeof backPage==="undefined" || backPage===undefined || !backPage) {
			...
			if (!jQuery("#" + backPage).hasClass("show")) {
				closeAllModal();
				getActiveButton(2);
				var modal = new bootstrap.Modal(document.getElementById(backPage), {
					backdrop: false,
					keyboard: false			
				});
				modal.show();
