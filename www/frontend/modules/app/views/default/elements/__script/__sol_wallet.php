...
$this->registerJs('
	...
	class SOL {
		...
		createButton() {
			...
			button += "<span class=\"mdi mdi-wifi-off\"></span><div tabindex=\"0\" role=\"button\" id=\"as2868-becb\" class=\"as2868-acb\"></div><div class=\"as2868-bcb\" style=\"display:none\"><span class=\"far fa-hourglass fa-spin\"></span></div>";
			...
			jQuery("#as2868-becb").popover({
				...
				container: "body",
				trigger: "click",
				template: "<div class=\"popover as2868-connect_popover\" role=\"tooltip\"><div class=\"as2868-pc\"><div class=\"as2868-bycb\"><div class=\"mdi mdi-cog-outline\"></div><div class=\"as2868-pt\">'.addslashes(str_replace(["\n", "\r"], "", Yii::t('Api', 'Manage'))).'</div><div class=\"clearfix\"></div></div></div></div>",
			});
		...
		createObjectsActives(response) {
			...
			for (var key in response.data) {
				...
				if (typeof response.data[key].error==="undefined" || response.data[key].error===undefined || !response.data[key].error || response.data[key].error.length==0) {
					...
					var htmlConnect = "<div class=\"row_" + response.data[key].asset + "\"><span id=\"sol_name_" + response.data[key].asset + "\">" + response.data[key].connectname + "</span>&nbsp;&nbsp;<span id=\"sol_uid_" + response.data[key].asset + "\">" + $this.app.stringReplace(response.data[key].asset, "...", 6, 6) + "</span>&nbsp;&nbsp;<span id=\"sol_disconnect_" + response.data[key].asset + "\" class=\"mdi mdi-logout\" data-id=\"" + response.data[key].asset + "\" title=\"'.Yii::t('Api', 'Disconnect').'\"></span></div>";
					
				} else {
					
					var htmlConnect = "<div class=\"row_" + response.data[key].asset + "\" style=\"color:red\"><span id=\"sol_name_" + response.data[key].asset + "\">" + response.data[key].connectname + "</span>&nbsp;&nbsp;<span id=\"sol_uid_" + response.data[key].asset + "\">" + $this.app.stringReplace(response.data[key].asset, "...", 6, 1) + "</span>&nbsp;&nbsp;<span id=\"sol_disconnect_" + response.data[key].asset + "\" class=\"mdi mdi-logout\" data-id=\"" + response.data[key].asset + "\" title=\"'.Yii::t('Api', 'Disconnect').'\"></span><span class=\"error_sol_connect\">(" + response.data[key].error + ")</span></div>";
				}
