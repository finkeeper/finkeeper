...
use backend\modules\tools\modules\exchange\assets\ModuleAsset;

ModuleAsset::register($this);

$this->title = Yii::t('Backend', 'Page Exchange');
...
$this->registerJs("
	jQuery(document).ready(function($) {
		...
		$('#add-crypto').on('click', function() {
			...
			var value = $(this).parents('.modal-body').find('#cryptocurr').val();

			$('#modal-crypto').modal('hide');
			
			if (!value) {
				
				$('#update-results').html('<div class=\"alert alert-danger mt-10\">".Yii::t('Backend', 'Missing cryptocurrency')."</div>');
				
			} else {
			
				$.ajax({
				url: '/tools/exchange/create',  
				type: 'POST',
				data: {value: value},
					...
					if (response) {
						try {
							...
							if (data.error) {
								...
								$('#update-results').html('<div class=\"alert alert-success mt-10\">".Yii::t('Backend', 'Success cryptocurrency')."</div>');
			...
			<div class="site-exchange theme_content">
				<div class="table-responsive">
					...
					<div class="button_container text-right">
						...
						<?php Modal::begin([
							'title' => Yii::t('Backend', 'Add cryptocurrency'),
							'size' => 'modal-lg',
							'id' => 'modal-crypto',
							'toggleButton' => [
								'label' => Yii::t('Backend', 'Add cryptocurrency'),
								'class' => 'btn btn-info add-crypto',
							],
						]); ?>

							<div class="row">
							
								<?=Html::textarea('cryptocurr', '', [
									'id' => 'cryptocurr',
									'class' =>  'form-control',
									'style' => 'min-height:20%',
								])?>
							...
							<div class="row">
								...
								<div class="text-left">
									...
									<?=Yii::t('Backend', 'Crypto Instruction')?>
							...
							<?=Html::Button(Yii::t('Backend', 'Add Crypto'),
								[
										...
										'buttons' => [
											'delete' => function ($url, $model) {
												...
												return Html::a(
													...
													Url::toRoute(['/tools/exchange/delete', 'id'=>$model->id_crupto])
												);
